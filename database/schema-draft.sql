-- Esquema preliminar para revisión.
-- No ejecutar en producción hasta revisar el Excel real o una muestra representativa.
...
CREATE TABLE entities (
    ...
    geocoding_status ENUM('not_required', 'pending', 'resolved', 'failed') NOT NULL DEFAULT 'pending',
    ...
    equality_protocol_status ENUM('yes_own', 'yes_external', 'in_progress', 'no', 'unknown') NULL,
    violence_protocol_status ENUM('yes_own', 'yes_external', 'in_progress', 'no', 'unknown') NULL,
    lopivi_status ENUM('yes_own', 'yes_external', 'in_progress', 'no', 'unknown') NULL,
...
CREATE TABLE entity_contacts (
    id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    entity_id BIGINT UNSIGNED NOT NULL,
    contact_type ENUM('phone', 'email', 'person') NOT NULL,
...
CREATE TABLE entity_social_links (
    id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    entity_id BIGINT UNSIGNED NOT NULL,
    platform ENUM('facebook', 'instagram', 'youtube', 'x', 'tiktok', 'other') NOT NULL,
    label VARCHAR(120) NULL,
    url TEXT NOT NULL,
    sort_order SMALLINT UNSIGNED NOT NULL DEFAULT 100,
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE media_files (
    id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    owner_type ENUM('entity', 'modality', 'facility') NOT NULL,
    owner_id BIGINT UNSIGNED NOT NULL,
    media_type ENUM('logo', 'photo', 'document', 'icon') NOT NULL,
...
CREATE TABLE imports (
    ...
    status ENUM('uploaded', 'previewed', 'processing', 'completed', 'completed_with_errors', 'failed') NOT NULL DEFAULT 'uploaded',
...
CREATE TABLE import_rows (
    ...
    status ENUM('pending', 'created', 'updated', 'skipped', 'error') NOT NULL DEFAULT 'pending',
